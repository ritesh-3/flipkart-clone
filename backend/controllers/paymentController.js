...
exports.paytmResponse = (req, res, next) => {
    ...
    if(process.env.SKIP_PAYTM_PROCESS === "TRUE"){
        const paymentRequestBody = {
            ...
            txnId: `TEST_${uuidv4()}`,        
            bankTxnId: "9876543210",  
            orderId: req.body.ORDERID ? req.body.ORDERID : "1234567890",  
            txnAmount:  req.body.TXNAMOUNT ? req.body.TXNAMOUNT : "0",        
            txnType: "Payment",        
            gatewayName: "Paytm",        
            bankName: "ABC Bank",       
            mid: req.body.MID ? req.body.MID : "1234567890",   
            paymentMode: "UPI",   
            refundAmt: "0.00",          
            txnDate: Date.now(),    
          };
          addPayment(paymentRequestBody);
          res.redirect(`${process.env.CLIENT_URL}/order/${paymentRequestBody.orderId}`)
          return
    }
        ...
        paytm.generateSignature(JSON.stringify(paytmParams.body), process.env.PAYTM_MERCHANT_KEY).then(function (checksum) {
            ...
            var post_req = https.request(options, function (post_res) {
                ...
                post_res.on('end', function () {
                    ...
                    res.redirect(`${process.env.CLIENT_URL}/order/${body.orderId}`)
                });
            });
